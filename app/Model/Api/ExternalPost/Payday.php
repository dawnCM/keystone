...
class Payday extends AppModel {
	...
	public $validate = array(
			...
			'LoanAmount' => array(
					'rule' => "/100|200|300|400|500|600|700|750|800|900|1000/",
					'message' => 'LoanAmount',
					'required' => true,
					'allowEmpty' => false),
			'FirstName' => array(
	...
	public function buildPost(){
		...
		$data = $this->data['Payday'];	
				
		if(isset($data['AgreePhone']) === false ){$data['AgreePhone']='';}
		if(isset($data['Address2']) === false){$data['Address2']='';}
		if(isset($data['CellPhone']) === false){$data['CellPhone']='';}
		...
		/*Removed because it apparently was added for no reason.
	...
	public function doNotMatch($check, $compare){
		...
		return ($this->data['Payday'][$compare] != $value);
	}
